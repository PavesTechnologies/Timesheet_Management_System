...
public class MonthlyUserReportService {
    ...
    public MonthlyUserReportDTO getMonthlyUserReport(Long userId, int month, int year) {
        ...
        // Exclude the internal project (projectId = -1) from this KPI only: its logged
        // hours are still counted and shown everywhere else (totals, donut, breakdown),
        // but it is not a real project, so it must not inflate "Current Active Projects".
        final long INTERNAL_PROJECT_ID = -1L;
        int activeProjectsCount = allEntries.stream()
                .map(TimeSheetEntry::getProjectId)
                .filter(Objects::nonNull)
                .filter(id -> id.longValue() != INTERNAL_PROJECT_ID)
